...
colnames(PI)<-list("pi1","pi2","pi3","pi4")
colnames(w)<-list("w1","w2","w3","w4")
colnames(v)<-list("v1","v2","v3","v4")
colnames(sigma2)<-list("sigma21","sigma22","sigma23","sigma24")
...
calc.convergence<-function(PI,w,v,sigma2){
  op <- par(mfrow=c(4,3))
  for(i in 1:3){cat(paste("convergence analysis of",i,"th pi\n"));calc.convergence.each(PI[,i],paste("pi",i))}
...
warm = floor(t.or/2)
apply(PI[warm:t.or,],2,mean)
sqrt(apply(w[warm:t.or,],2,mean))
sqrt(apply(v[warm:t.or,],2,mean))
sqrt(apply(sigma2[warm:t.or,],2,mean))
