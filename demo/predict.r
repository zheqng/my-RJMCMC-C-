...
plot.z<-function(z){
  ...
  N.tmp=dim(z)[1]
  curve_num = dim(z)[2]
  plot(-100,-100,xlim=c(1,N.tmp),ylim = c(1,curve_num))
  for(i in 1:curve_num){
    for(iter in 1:dim(z)[1])
    points(iter,i,col=color[z[iter,i]],pch=8,cex=0.5)
    
  }
}

plot.label<-function(traindata,z){
  N.tmp = dim(z)[1]
  color_label=rainbow(5)
  plot(-100,-100,xlim=range(traindata$X),ylim=range(traindata$Y))
  for (ii in 1:(traindata$curve.num))
    lines(traindata$X[ii,],traindata$Y[ii,],col = color_label[z[N.tmp,ii]],type = "l")
}
