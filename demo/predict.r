...
plot.z<-function(z,N.paper=20000){
  # N.paper=N
  color=c("red","blue","green")
  N.tmp=dim(z)[1]
  plot(-100,-100,xlim=c(1,N.paper),ylim = c(1,9),alab="iteration",ylab="classification")
  for(i in 1:9){
    for(iter in 1:dim(z)[1])
      points(ceiling(iter/N.tmp*N.paper),i,col=color[z[iter,i]])
