...
N.10 = which(datha.K==9)
# permu.theta<-function(N.10,traindata,theta,N){
  ortheta = vector("list",length(N.10))
  library(combinat)
  perma=permn(9)
  t.or=1
  library('foreach')
  ...
  foreach (t = N.10)%do%
  {
    entropies=rep(0,factorial(9))
    foreach (m  = 1:(traindata$curve.num))%do%
    {
      log.P = as.vector(rep(0,9))
      foreach(k = 1:9)%do%
      {log.P[k] = log.likelihood.micro(traindata,theta[[t]],m,k)}
