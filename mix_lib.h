...
struct pq_point
{
        ...
        void insertPre_seq(int k, pq_point Elem_mix)
        {
                ...
                else
                {
                        ...
                        if (k == K) { pi(k) = Elem_mix.pi(0); }
                        else {pi.tail(K - k) = pi.segment(k, K - 1); pi(k) =Elem_mix.pi(0); }
                        pi(k) = Elem_mix.pi(0);

                        tmp = w; w.resize(K+1); w.head(K) = tmp;
                        if (k == K) { w(k) = Elem_mix.w(0); }
                        else {w.tail(K - k) = w.segment(k, K - 1); w(k) =Elem_mix.w(0); }
                        w(k) = Elem_mix.w(0);

                        tmp = v; v.resize(K+1); v.head(K) = tmp;
                        if (k == K) { v(k) = Elem_mix.v(0); }
                        else {v.tail(K - k) = v.segment(k, K - 1); v(k) =Elem_mix.v(0); }
                        v(k) = Elem_mix.v(0);

                        tmp = sigma2; sigma2.resize(K+1); sigma2.head(K) = tmp;
                        if (k == K) { sigma2(k) = Elem_mix.sigma2(0); }
                        else {sigma2.tail(K - k) = sigma2.segment(k, K - 1); sigma2(k) =Elem_mix.sigma2(0); }
                        sigma2(k) = Elem_mix.sigma2(0);
